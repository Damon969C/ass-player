...
2. 普通窗口只做窗口内沉浸，不改变外层窗口大小；外层窗口已经最大化，或在沉浸期间被最大化时，才调用 `showFullScreen` 进入系统真全屏。
3. 底部控制条在沉浸状态下从纵向布局移出并覆盖在视频底部，显示/隐藏不会改变视频尺寸。鼠标或键盘活动会显示控制条并重置 2 秒计时器；超时后控制条和光标隐藏。
4. 全屏按钮再次点击或按 `Esc` 退出。系统真全屏返回最大化，窗口内沉浸保留当前外层窗口状态；mpv 的 `videoHost` 原生窗口不重建，因此播放不中断。
...
3. 进度、播放暂停、音量、静音、快捷键和 OSD 可用；窗口内沉浸、最大化后的系统真全屏、2 秒控制条隐藏及 `Esc` 退出行为正确。

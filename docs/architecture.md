...
1. 顶部播放/暂停按钮后的“重复”开关启用逐条听力练习；关闭时沿用普通播放的全部行为。
2. `repeatCueId_` 固定当前练习字幕。这样播放停在重复边界后，左右键仍按该字幕的列表索引选择真正的上一条/下一条，而不是再次命中刚播完的字幕。
3. `subtitleCueRepeatBoundary` 优先使用下一条字幕的起点；若它晚于当前字幕结束时间 2 秒以上，则改用“当前字幕结束时间 + 2 秒”，最后再受媒体总时长约束。重叠字幕直接在下一条起点暂停。
4. 重复模式将状态轮询从 250ms 临时提高到 50ms；抵达重复边界后先发送 `pause=yes`，再精确回定位到该边界，避免轮询延迟让画面越过停止点。
5. 空格在重复模式下从当前练习字幕起点播放；播完仍停在重复边界。`K`、播放/暂停按钮、字幕点击、进度条、音量与沉浸播放逻辑保持原行为。
6. 当前字幕没有显式结束时间时使用下一条字幕起点；末条字幕没有下一条时使用“当前字幕结束时间 + 2 秒”，缺少有效结束时间则使用媒体总时长。

### 沉浸播放

1. 音量条右侧的全屏按钮切换 `MainWindow` 的沉浸状态；进入后隐藏顶部工具栏和右侧字幕栏，让 `videoShell` 填满可用客户区。
2. 普通窗口只做窗口内沉浸，不改变外层窗口大小；外层窗口已经最大化，或在沉浸期间被最大化时，创建独立的 `immersiveFullscreenWindow` 承载播放器并进入系统真全屏，主窗口始终保持最大化状态。
3. 底部控制条在沉浸状态下从纵向布局移出并覆盖在视频底部，显示/隐藏不会改变视频尺寸。鼠标或键盘活动会显示控制条并重置 2 秒计时器；超时后控制条和光标隐藏。
4. 全屏按钮再次点击或按 `Esc` 退出。播放器面板从独立全屏窗口移回原主窗口，窗口内沉浸则保留当前外层窗口状态；mpv 的 `videoHost` 原生窗口 ID 不变，因此播放不中断。
5. 真全屏状态只属于独立播放窗口，主窗口不再经历“最大化 → 真全屏 → 最大化”的状态往返。这样既没有正常窗口态中转动画，也不会触发 Qt/Windows 清空无边框主窗口恢复矩形的缺陷；退出播放全屏后，右上角窗口化仍沿用原本未被破坏的最大化恢复链。
6. 普通窗口为原生 `videoHost` 设置与界面一致的圆角 `QRegion`，真实裁剪 mpv 子窗口而不只绘制样式圆角；宿主缩放时重算区域，沉浸模式清除区域，退出沉浸后恢复。

## 调试重点

- mpv 没有嵌入：检查 `videoHost_->winId()` 是否创建、mpv 参数是否包含 `--wid`、Windows 下是否被其他窗口层级遮挡。
- 视频黑色直角越过圆角：检查 `updateVideoHostMask` 是否在 `videoHost` 尺寸变化和沉浸状态切换后执行；样式表 `border-radius` 本身不能裁剪原生 mpv 子窗口。
- 真全屏退出后无法窗口化：检查系统真全屏是否仅施加于 `immersiveFullscreenWindow`，不要对 `MainWindow` 调用 `showFullScreen`；同时确认退出时播放器面板已移回主窗口且其原生 `videoHost` ID 未改变。
...
3. 进度、播放暂停、音量、静音、快捷键和 OSD 可用；重复模式逐条停播、空格重播和左右字幕切换正确；窗口内沉浸、最大化后的系统真全屏、2 秒控制条隐藏及 `Esc` 退出行为正确。

...
2. `repeatCueId_` 固定当前练习字幕。这样播放停在字幕终点后，左右键仍按该字幕的列表索引选择真正的上一条/下一条，而不是再次命中刚播完的字幕。
3. 重复模式将状态轮询从 250ms 临时提高到 50ms；抵达字幕结束时间后先发送 `pause=yes`，再精确回定位到字幕终点，避免轮询延迟让画面停在下一句。
4. 空格在重复模式下从当前练习字幕起点播放；播完仍停在终点。`K`、播放/暂停按钮、字幕点击、进度条、音量与沉浸播放逻辑保持原行为。
5. 字幕没有显式结束时间时，纯导航层依次使用下一条字幕起点或媒体总时长作为停止边界。
